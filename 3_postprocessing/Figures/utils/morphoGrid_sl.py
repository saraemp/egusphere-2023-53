...
class morphoGrid:
    ...
    def loadHDF5(self, timestep=0):
        ...
        """
        with open('%s/../xmf/tin.time%s.xmf'%(self.folder, timestep),'r') as f:
            lines = f.read().split("\n")
        word = 'Function'
        for i,line in enumerate(lines):
            if word in line:
                ll = line[71:]
                self.sl = float(ll[0:ll.find('"')])

        df = h5py.File('%s/tin.time%s.hdf5'%(self.folder, timestep), 'r')
        coords = np.array((df['/coords']))
        cumdiff = np.array((df['/cumdiff']))
        discharge = np.array((df['/discharge']))
        flexure = np.array((df['/cumflex']))
        self.cells = np.array((df['/cells']))
        self.area = np.array((df['/area']))
        self.vertices = coords.copy()
        self.fa = discharge.copy()
        self.ed = cumdiff.copy()
        ...
        self.flexure = np.reshape(fi,(self.ny,self.nx))
